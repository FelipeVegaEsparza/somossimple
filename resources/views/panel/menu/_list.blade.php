    ...
    <div class="card p-4 mb-3 flex items-center gap-4">
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
                ...
                @if ($item->featured)
                    <span class="badge bg-[#fff6e0] text-warn">Destacado</span>
                @endif
                @if (! $item->active)
            ...
            @endif
            <p class="text-sm font-semibold mt-0.5">{{ $item->priceDisplay() ?? 'Sin precio' }}</p>
        </div>

        <div class="flex items-center gap-1 shrink-0">
            <form method="POST" action="{{ route('panel.menu.toggle', $item) }}">
                @csrf
                <button type="submit" class="btn-ghost text-ink-soft" title="{{ $item->active ? 'Desactivar' : 'Activar' }}">
                    {{ $item->active ? 'Desactivar' : 'Activar' }}
                </button>
            </form>
            <a href="{{ route('panel.menu.edit', $item) }}" class="btn-ghost">Editar</a>
            <form method="POST" action="{{ route('panel.menu.destroy', $item) }}" onsubmit="return confirm('¿Eliminar este plato?')">
                @csrf
                @method('DELETE')
